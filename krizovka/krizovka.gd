...
func _unhandled_input(event):
	var diakritika = ["é",-1,"ľ","š","č","ť","ž","ý","á","í"]
	if event is InputEventKey:
		if event.pressed:
			if selected != null:
				var pismeno = event.keycode
				if (pismeno >= 65 and pismeno <= 65+26):
					selected.label_pismeno.text = char(pismeno)
				if pismeno >= 48 and pismeno <= 57 and pismeno != 49:
					selected.label_pismeno.text = diakritika[pismeno-48]
				if char(pismeno) == ";":
					selected.label_pismeno.text = "ô"
				if char(pismeno) == "[":
					selected.label_pismeno.text = "ú"
				if char(pismeno) == "]":
					selected.label_pismeno.text = "ä"
				if pismeno == 92:
					selected.label_pismeno.text = "ň"
				if pismeno == KEY_BACKSPACE or pismeno == KEY_DELETE:
